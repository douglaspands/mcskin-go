...
1. **Understand the request, derive change name & create feature branch (Mandatory First Action)**
   ...
   **MANDATORY FIRST COMMAND (Branch Isolation)**:
   ...
   ```
   This ensures that all subsequent planning, files, or changes are strictly isolated from `main` and can be reverted immediately if anything deviates.
...
7. **Annotate tasks with model recommendations**
   ...
   **Annotation format** — append inline after each task checkbox line:
   ...
   - [ ] Implement `internal/skin/validate.go` — single file, complete spec
         <!-- model: flash | signal: 1-file mechanical implementation -->

   - [ ] Write unit tests for skin validation — mocked, single package
         <!-- model: flash | signal: 1-file, spec complete -->

   - [ ] Refactor converter to use new validator — multi-file coordination
         <!-- model: flash | signal: integration task, 3 files -->

   - [ ] PO/QA review with feature-qa-reviewer — broad judgment required
         <!-- model: flash | signal: architecture + QA eval -->
   ```

   **Cross-harness note**: also write the Claude Code equivalent in the annotation. Use the short dispatch enum (`flash`/`sonnet` — never `haiku`, disabled for this project) that Claude Code's `Agent` tool `model` parameter accepts — see `.agents/skills/model-selection/SKILL.md` Cross-Harness Model Mapping table for the full model IDs used in logging:

   ```markdown
   <!-- model: flash (AGY) / sonnet (Claude) | signal: ... -->
   ```

   **Rules for annotation:**
   - Use the mcskin-Specific Assignments table first (exact match wins)
   - Fall back to the generic Decision Table if no exact match
   - Never annotate with `flash_lite`/`haiku` (Cheap/fast tier) — disabled for this project; `flash`/`sonnet` is the floor
   - When uncertain between two tiers, annotate with the cheaper of the two *allowed* tiers and add `(verify)`
   - Never annotate with `inherit` — always resolve to a concrete tier
   ...
   |---|---|---|---|---|
   | Task 1 description | mid  | flash | sonnet | 1-file mechanical |
   | Task 2 description | mid  | flash | sonnet | multi-file prose  |
   | Task 3 description | deep | flash | sonnet | QA eval / design  |
...
8. **Analyze task dependency graph & mark parallelizable waves (Graph Engineering)**
   ...
   **Annotate each task's wave** inline, next to the existing model annotation:
   ```markdown
   - [ ] Implement `internal/skin/validate.go` — single file, complete spec
         <!-- model: flash (AGY) / sonnet (Claude) | signal: 1-file mechanical implementation | wave: 1 -->
   ```
