...
- Offload generation of each variation to a subagent, using the model the user has selected/configured in the harness (no per-task override — see AGENTS.md's Parallel Dispatch & Model Selection section):
  - **Antigravity**: a `research` or `self` subagent dispatched with `Model: inherit`.
  - **Claude Code**: the `Agent` tool with `subagent_type: "general-purpose"` (or `"fork"` if the variation needs this conversation's context).
