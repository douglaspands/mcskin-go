...
- [x] **Rodada 4 — QR Code sem Cortes, Ferramenta Trocar Cor pontual e Padronização de Cancelar no Design System**:
  - QR Code: expandido `.drawer-qr-preview` (140x140px) e aplicado `max-width: 100% !important; object-fit: contain;` em canvases/imagens para eliminar corte dos finder patterns em mobile e desktop.
  - Ferramenta Trocar Cor (`toolRecolor`): corrigido raycast em `three.min.js` (filtragem de camada Base vs Overlay no clique) e `recolorAll` em `editor2d.js` (ignora áreas transparentes e substitui unicamente os pixels correspondentes à cor clicada).
  - Design System de Botões: padronizados todos os botões de cancelamento de modais com `.btn-minecraft-secondary` e `.btn-modal-cancel` (gradiente pedra chanfrada), e atualizado integralmente `.agents/skills/ui-component-library-ref/SKILL.md` com os novos tokens, classes, breakpoints e índice de módulos.
- [x] **Rodada 5 — Ativação da Visão 2D e Correção das Faces dos Braços nas Costas**:
  - Visão 2D: restaurada a pílula de alternância 3D/2D (`#btnMode3D` e `#btnMode2D`) na barra superior, montado `#wrapper2D` com `#editor2DCanvas` (512x512) no palco e conectados os eventos em `palette.js` com isolamento em `editor3d.js`.
  - Braços escuros nas costas: corrigido `loadTemplate` em `editor2d.js` para cobrir todas as 4 faces da manga e 4 faces da pele no braço direito e no braço esquerdo em Steve e Alex, eliminando áreas transparentes/escuras nas costas do boneco.
