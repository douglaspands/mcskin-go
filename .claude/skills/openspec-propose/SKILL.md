...
7. **Annotate tasks with model recommendations**
   ...
   **Annotation format** — append inline after each task checkbox line:
   ...
   - [ ] PO/QA review with feature-qa-reviewer — broad judgment required
         <!-- model: pro | signal: architecture + QA eval -->
   ```

   **Cross-harness note**: also write the Claude Code equivalent in the annotation. Use the short dispatch enum (`sonnet`/`opus` — never `haiku`, disabled for this project) that Claude Code's `Agent` tool `model` parameter accepts — see `.agents/skills/model-selection/SKILL.md` Cross-Harness Model Mapping table for the full model IDs used in logging:
   ...
   **Rules for annotation:**
   ...
   | Task 3 description | deep | pro   | opus   | QA eval / design  |
